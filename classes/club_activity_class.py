...
class ClubActivity:
    ...
    def get_activity_byId(activityId):
        with dbapi2.connect(current_app.config['dsn']) as connection:
            ...
            statement = """SELECT * FROM CLUBACTIVITIES WHERE CLUBACTIVITIES.ID = %s"""
            cursor.execute(statement, [activityId])
            db_activity = cursor.fetchall()
            activity = ClubActivity(db_activity[0][1], db_activity[0][2], db_activity[0][5], db_activity[0][3],
    ...
    def delete_activity_byId(activityId):
        with dbapi2.connect(current_app.config['dsn']) as connection:
            cursor = connection.cursor()
            statement = """DELETE FROM CLUBACTIVITIES WHERE CLUBACTIVITIES.ID = %s"""
            cursor.execute(statement, [activityId])